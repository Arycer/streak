...
export class StatsService {
   ...
   */
  static async calculateCurrentStreak(task: Task, completions: TaskCompletion[]): Promise<number> {
   ...
   */
  static async calculatePreviousStreak(task: Task, completions: TaskCompletion[]): Promise<number> {
    const today = new Date().toISOString().split('T')[0];
    const completionDates = new Set(completions.map(c => c.completion_date));
   ...
   */
  static async calculateLongestStreak(task: Task, completions: TaskCompletion[]): Promise<number> {
    const completionDates = new Set(completions.map(c => c.completion_date));
    ...
    for (const date of allDates.reverse()) { // Empezar desde más antiguo
      // No contar días antes de la creación de la tarea
   ...
   */
  static async getCompletionCount(task: Task, completions: TaskCompletion[], days: number = 30): Promise<number> {
   ...
   */
  static async isStreakBrokenOnDay(task: Task, day: string, completions: TaskCompletion[]): Promise<boolean> {
  ...
  static async calculateTaskStreakStats(task: Task): Promise<StreakStats> {
    ...
    const [currentStreak, previousStreak, longestStreak, completionCount] = await Promise.all([
    ...
    const isStreakBroken = await this.isStreakBrokenOnDay(task, today, completions);
  ...
  static async getUserStats(): Promise<UserStats> {
    ...
    const consistencyPercentage = await this.calculateConsistencyPercentage(tasks);
   ...
   */
  static async calculateConsistencyPercentage(tasks: Task[], days: number = 30): Promise<number> {
    ...
    for (const date of allDates) {
      ...
      const tasksForDay = tasks.filter(task => 
        task.days.includes(dayOfWeek) && date >= task.created_at
      );
  ...
  static async getDailyHistory(days: number = 30): Promise<DailyHistoryItem[]> {
    ...
    for (const date of allDates) {
      ...
      for (const task of missedTasks) {
        const completions = await CompletionService.getTaskCompletions(task.id);
        const isStreakBroken = await this.isStreakBrokenOnDay(task, date, completions);
        if (isStreakBroken) {
  ...
  static getDayAbbreviations(days: string[]): string {
    ...
    return days.map(day => dayMap[day] || day.charAt(0).toUpperCase()).join(', ');
  }

  /**
   * Obtener clases de color para las tareas
   */
  static getColorClasses(color: string): { bg: string; border: string; text: string } {
